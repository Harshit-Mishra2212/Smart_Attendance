...
def api_add_student():
    ...
    return jsonify({"status": "success", "message": f"Student {data['name']} added (or ignored if duplicate)."})

@app.route("/add_teacher", methods=["POST"])
def api_add_teacher():
    data = request.json
    add_teacher(data["name"], data["department"])
    return jsonify({"status": "success", "message": f"Teacher {data['name']} added (or ignored if duplicate)."})

@app.route("/add_class", methods=["POST"])
def api_add_class():
    data = request.json
    add_class(data["subject_name"], data["teacher_id"], data["year"], data["class"])
    return jsonify({"status": "success", "message": f"Class {data['subject_name']} added (or ignored if duplicate)."})

@app.route("/enroll_student", methods=["POST"])
def api_enroll_student():
    data = request.json
    enroll_student(data["student_id"], data["class_id"])
    return jsonify({"status": "success", "message": f"Student {data['student_id']} enrolled in class {data['class_id']}."})

@app.route("/mark_attendance", methods=["POST"])
def api_mark_attendance():
    data = request.json
    mark_attendance(data["class_id"], data["student_id"], data["date"], data["status"])
    return jsonify({"status": "success", "message": f"Attendance marked for student {data['student_id']}."})
...
if __name__ == "__main__":
    app.run(debug=True, host="0.0.0.0")  # ✅ host=0.0.0.0 for access from other devices
